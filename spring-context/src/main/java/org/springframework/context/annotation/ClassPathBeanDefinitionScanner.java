...
public class ClassPathBeanDefinitionScanner extends ClassPathScanningCandidateComponentProvider {
	...
	/**
	 * 为容器创建一个ClassPathBeanDefinitionScanner，并指定是否使用默认的扫描过滤规则，
	 * 即spring默认扫描配置@Component、@Repository、@Service、@Controller注解的bean，
	 * 同时也支持JavaEE6的@ManangedBean和JSR-330的@Named注解
	 *
	 * Create a new {@code ClassPathBeanDefinitionScanner} for the given bean factory.
	...
	public ClassPathBeanDefinitionScanner(BeanDefinitionRegistry registry, boolean useDefaultFilters,
		...
		if (useDefaultFilters) {
			// 初始化默认的需要被自动注册到ioc容器的注解标识list（@Component、@ManagedBean、@Named等）
	...
	protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
		...
		for (String basePackage : basePackages) {
			...
			for (BeanDefinition candidate : candidates) {
				...
				// 根据beanName检查指定的bean是否需要在容器中注册，或者是否在容器中存在冲突
				if (checkCandidate(beanName, candidate)) {
					...
					// 最后注册BeanDefinition，这样就将包下的所有BeanDefinition都注册完成了
